...
let checkString = motto.includes('cool');
...
let updMotto = motto.replace('iSchool', 'Information School');
console.log(updMotto);
...
let ratio = (updMotto.length / motto.length * 100);
let ratioPercentage = ratio.toFixed(2) + '%';
...
let numSort = numbers.sort(function(a, b){return a - b});
let numMedian = numSort[(numSort.length - 1) / 2];
...
//the current property value!). What do you get?   Answer: got undefined
...
//Chrome developer console?   Answer: the previously logged array updated with new
//values as well.
