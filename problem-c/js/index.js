...
//Testing function `extractOpponent()`
//console.log(extractOpponent(huskyGames2016[2]));
...
let huskiesOpponents = huskyGames2016.map(extractOpponent);

console.log(huskiesOpponents);
...
let huskiesGamesLost = huskyGames2016.filter(huskiesLost);

console.log(huskiesGamesLost);

//Log out an array of opponents that UW lost to. Hint: Use the `.map()` method 
//to extract the opponent names!
let opponentsHuskiesLostTo = huskiesGamesLost.map(extractOpponent);

console.log(opponentsHuskiesLostTo);
...
huskiesGamesLost.forEach(function(game) {
...
let huskiesFumbleGames = huskyGames2016.filter(function(game) {
  let gamesFumble = 0;
  if (game["fumbles"] >= 1) {
    gamesFumble += 1;
  }
...
console.log(huskiesFumbleGames.length);
...
function makeCombinedFilter(cbOne, cbTwo) {
  let combinedFilter = function(game) {
    return cbOne(game) && cbTwo(game);
  };
...
let fumbledAndLostFilter = makeCombinedFilter(huskiesLost, function fumbleGames(game) {
...
let huskiesLostWithFumbles = huskyGames2016.filter(fumbledAndLostFilter);
console.log(huskiesLostWithFumbles);
