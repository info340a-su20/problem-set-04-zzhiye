...
//the 4 in the function is coerced to a string, thus logging '84'
...
*/
function compoundInterest(begBalance, interest, numOfYears) {
  let totalBalance = begBalance*Math.pow(Math.E, interest*numOfYears);

  return totalBalance;
}

console.log(compoundInterest(1000, .06, 5));
...
function getHighCard(hand) {
  ...
  for (let i=0; i<hand.length; i++) {
    if (hand[i].rank > highCard.rank) {
      highCard = {"rank": hand[i].rank, "suit": hand[i].suit};
    }
  }
  return highCard;
}
